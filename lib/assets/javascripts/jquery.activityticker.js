    ...
    refresh : function(activity_id) {
      ...
      $.ajax({
        ...
        complete : function(data, status){
          self._handleResponse( data.responseText, function(){ 
            $.each($.parseJSON(data.responseText), function(index, activityItem){
              self.list.prepend(ich.activity_item(activityItem));              
            });
          } );
    ...
    _handleScroll : function(eventData) {
      ...
      if (isScrollable) {
        ...
        $.ajax({
          ...
          complete : function(data, status) {
            self._handleResponse( data.responseText, function(){
              $.each($.parseJSON(data.responseText).reverse(), function(index, activityItem){
                $(ich.activity_item(activityItem)).insertBefore( self.loader );              
              });
            });
            self.loading = false;
          },
